...
def MFACheck(user, password, tenant, client, useragent, savetokens):
    for resource, scope in Scopes.Scope.items():
        response = None

        response = Token.ResourceOwnerPasswordCredentials(user, password, tenant, client, scope, useragent)
        ...
        if resource == "o365":
            for ua_name, ua_value in UserAgents.Agent.items():
                ...
                response = Token.ResourceOwnerPasswordCredentials(user, password, tenant, client, scope, ua_value)
...
def main(userlist, passwordlist, tenant, client, scope, useragent, mfacheck, savetokens):
        ...
        for password in passwords:
            try:
                ...
                if response.status_code == 200:
                    ...
                    if mfacheck:
                        MFACheck(user, password, tenant, client, useragent, savetokens)
                    ...
                    if mfacheck:
                        MFACheck(user, password, tenant, client, savetokens)
                        SMTPBasicAuthCheck(user, password)
